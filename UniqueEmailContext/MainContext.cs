...
namespace UniqueEmailContext
{
    public class MainContext
    {
        ...
        public HashSet<string> GetUniqueEmails(List<string> emailArray)
        {
            ...
            {
                foreach (string eachEmail in emailArray)
                {
                    string result = null;
                    Boolean isValidDomain = false;

                    // check if trimmed email is empty
                    if (String.IsNullOrEmpty(eachEmail.Trim()))
                    {
                        continue;
                    }
                    else
                    {
                        string trimEachEmail = eachEmail.Trim();
